...
def load_data(where  = "./flowers" ):
    ...
    This function receives the location of image files, applies the necessary transformations (rotations,flips,normalizations and crops) and converts the images to tensor in order to be able to fed into the neural network
    ...
    #Apply the required transformations to the test dataset in order to maximize the efficiency of the learning
    #process
    
    
    train_transforms = transforms.Compose([transforms.RandomRotationa(50),
    ...
    train_data = datsets.ImageFolder(train_dir, transform=train_transforms)
    validation_data = datasets.ImageFolder(train_dir, transform=train_transforms)
    test_data = datasets.ImageFolder(test_dir ,transform = test_transforms)
    
    # TODO: Using the image datasets and the trainforms, define the dataloaders
    # The data loaders are going to use to load the data to th NN(no shit Sherlock)
...
def nn_setup(structure='densenet121',dropout=0.5, hidden_layer1 = 120,lr = 0.001,power=gpu):
    '''
    Arguments: The architecture for the network(alexnet,densenet121,vgg16), the hyperparameters for the network (hidden layer 1 nodes, dropout and learning rate) and whether to use
    ...
    for param in model.parameters():
       ...
       optimizer = optim.Adam(model.classifier.parameter(), lr )
 ...
 def train_network(model, criterion, optimizer, epochs = 3, print_every=20, loader=trainloader, power='gpu':
            ...
            if steps % print_every == 0       
                ...
                for ii, (inputs2,labels2) in enumerate(vloader):   
                    ...
                    if torch.cuda.is_available():
                        inputs2, labels2 = inputs2.to('cuda:0') , labels.to('cuda:0')
                        model.to('cuda:0')
    ...
    print("Dear User I the ultimate NN machine trained your model. It required")
...
def save_checkpoint(path='checkpoint.pth',structure ='densenet121', hidden_layer1=120,dropout=0.5,lr=0.001,epochs=12):
    ...
    This function saves the model at a specified by the use path
...
def process_image(image_path):
    ...
    This function opens the image using the PIL package, applies the necessary transformations and returns the image as a tensor ready to be fed to the network
    ...
    make_img_good = transforms.Compose([ # Here as we did with the training data we will define a set of
